...
- Stage: plan.
- Branch: `incant/0004-pmtiles-overlay-contract`.
- Next step: after approval, run `/incant:implement 0004` and begin `0004-P1`.
- Blockers: none.
- Key decisions:
...
- `app/models/walking_path.rb` — first-class published walking path model with PostGIS line geometry validation.
- `db/migrate/<timestamp>_create_walking_paths.rb` — Rails-generated migration that creates the `walking_paths` table with LineString/MultiLineString geography, publication fields, labels, slug, and indexes.
- `db/schema.rb` — Rails schema update after the migration.
- `test/models/walking_path_test.rb` — model validations/scopes for published walking paths and geometry type requirements.
- `test/fixtures/walking_paths.yml` — fixture records for local/test smoke checks.
- `app/controllers/admin/walking_paths_controller.rb` — admin CRUD controller for walking path metadata, publication state, deletion, and GeoJSON geometry upload/paste.
- `app/views/admin/walking_paths/index.html.erb` — admin list page with create/edit/delete links and publication status.
- `app/views/admin/walking_paths/new.html.erb` — admin page for creating a walking path.
- `app/views/admin/walking_paths/edit.html.erb` — admin page for editing a walking path.
- `app/views/admin/walking_paths/_form.html.erb` — shared admin form for labels, slug, publication state, description, and GeoJSON geometry upload/paste.
- `test/controllers/admin/walking_paths_controller_test.rb` — admin request tests for CRUD, publish/unpublish, GeoJSON upload/paste, and invalid geometry errors.
- `config/routes.rb` — add `admin/walking_paths` routes.
...
## Phase 0004-P1 — contract and walking path source model
Goal: land the committed PMTiles contract artifact outside `/docs/` and the first-class Rails/PostGIS source for published walking paths.

- [ ] Read `db/schema.rb`, `app/models/application_record.rb`, `app/models/area.rb`, `app/models/cluster.rb`, `app/models/region.rb`, `app/models/poi.rb`, `app/models/poi_route.rb`, and `lib/tasks/mapbox.rake` before editing.
...
- [ ] Run `bin/rails generate migration CreateWalkingPaths` and edit the generated `db/migrate/<timestamp>_create_walking_paths.rb` to create `walking_paths` with `name`, `name_en`, `slug`, `published` default false/not null, `description`, `geometry` as SRID 4326 geography accepting LineString/MultiLineString, timestamps, a unique slug index, a GIST geometry index, and a published index.
- [ ] Add `app/models/walking_path.rb` with `audited`, a `published` scope, normalized text fields, slug presence/uniqueness validation for published records, geometry presence validation for published records, and a custom geometry-type validation that accepts only LineString and MultiLineString.
- [ ] Add `test/fixtures/walking_paths.yml` with one published LineString path and one unpublished path using SRID 4326 WKT-compatible fixture values.
- [ ] Add `test/models/walking_path_test.rb` covering published scope, required slug/geometry for published records, permitted LineString/MultiLineString geometry, rejected Point geometry, and unpublished drafts not being exported.
- [ ] Read `config/routes.rb`, `app/controllers/admin/pois_controller.rb`, `app/views/admin/pois/index.html.erb`, and `app/views/admin/pois/_form.html.erb` before editing admin UI files.
- [ ] Add `resources :walking_paths` under the existing `admin` namespace in `config/routes.rb`.
- [ ] Add `app/controllers/admin/walking_paths_controller.rb` with `index`, `new`, `create`, `edit`, `update`, and `destroy`; permit `name`, `name_en`, `slug`, `published`, `description`, pasted GeoJSON text, and uploaded GeoJSON file parameters; parse only GeoJSON LineString/MultiLineString feature or geometry input into `WalkingPath.geometry`; and show validation errors without losing form input.
- [ ] Add admin views `app/views/admin/walking_paths/index.html.erb`, `new.html.erb`, `edit.html.erb`, and `_form.html.erb` matching the existing admin style, with fields for metadata, published status, a GeoJSON textarea, a GeoJSON file upload control, current geometry preview as GeoJSON text, and destructive delete confirmation.
- [ ] Add `test/controllers/admin/walking_paths_controller_test.rb` covering index access, create from pasted GeoJSON LineString, update from uploaded GeoJSON MultiLineString, publish/unpublish metadata changes, destroy, and invalid Point GeoJSON rendering a clear validation error.

**Quality gate:** `bin/rails test test/models/walking_path_test.rb test/controllers/admin/walking_paths_controller_test.rb` → all walking path model and admin UI tests pass after the migration/schema update.
...
- [ ] Implement `problems` point export from published areas with required `problemId`, `areaId`, `areaSlug`, `name`, `grade`, `steepness`, `featured`, and optional `nameEn`, `popularity`, `landing`, `height`, `parentProblemId`.
...
- [x] Requirement 3 maps to P1 walking path migration/model/tests and P2 walking path export.
- [x] Requirement 18 maps to P1 admin walking path routes/controller/views and controller tests.
