...
namespace :josm_legacy do
  task geojson: :environment do
    ...
    file_name = Rails.root.join("..", "#{BRAND_CONFIG[:slug]}-maps", "josm", "area-#{area_id}", "area-#{area_id}-data.geojson")
  ...
  task metadata: :environment do
    ...
    File.open(Rails.root.join("..", "#{BRAND_CONFIG[:slug]}-maps", "josm", "area-#{area_id}", "area-#{area_id}-metadata.geojson"), "w") do |f|
      f.write(geo_json)
    end
