class SearchController < ApplicationController
  ...
  def perform_problem_search(query, show_unpublished)
    tsearch_results = Problem.tsearch_search(query).with_unpublished(show_unpublished).includes(:area).limit(10).to_a
    return tsearch_results if tsearch_results.present?

    trigram_low_results = Problem.trigram_search_low_threshold(query).with_unpublished(show_unpublished).includes(:area).limit(9).to_a
    trigram_low_results
  end
